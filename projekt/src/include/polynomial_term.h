...
template <typename T> Term<T>::Term(const T &c, const int &p) : coefficient(c) {
    assert(p >= 0);
    power = p;
}

template <typename T> void Term<T>::setPower(int power) {
    assert(power >= 0);
    this->power = power;
}

template <typename T> bool Term<T>::operator==(const Term &other) const {
